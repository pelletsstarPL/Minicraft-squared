...
public class Menu {
	...
	public static final int LIMIT_TYPING_SEARCHER = 22;
	...
	public static boolean searcherBarActive = false;
	...
	public void tick(InputHandler input) {
		...
		if (useSearcherBar) {
			...
			if (searcherBarActive) {
				//if(letters.indexOf(input.keyPressed(this.typingSearcher,null))!=-1) {
	...
	public void render(Screen screen) {
		bg=20;
		if(Settings.get("coloredgui").equals(true))
			switch(title){
				case "Have:": case "Main Menu Options": case "World Options":bg=21;break; //22
				case "Cost:": bg=22;break; //23
				case "Inventory": case "World Gen Options":bg=23;break; //24
				case "You died! Aww!": case "Help":bg=25;break;
				case "Chest":bg=27;break;
				case "Skins":bg=24;break;
				case "Dungeon Chest":bg=26;break;
				case "Workbench": case "Crafting": case "Anvil": case "Enchanter": case "Furnace": case "Oven": case "Loom": case "Stonecutter":bg=19;break;
			}
		renderFrame(screen);
		// render the title
		if(title.length() > 0) {
			...
			if (drawVertically) {
				for (int i = 0; i < title.length(); i++) {
					if (hasFrame) screen.render(titleLoc.x, titleLoc.y + i * Font.textHeight(), 3 + bg * 32, 0, 3);
					Font.draw(title.substring(i, i + 1), screen, titleLoc.x, titleLoc.y + i * Font.textHeight(), titleColor);
				}
			} else {
				for (int i = 0; i < title.length(); i++) {
					if (hasFrame) screen.render(titleLoc.x + i * Font.textWidth(" "), titleLoc.y, 3 + bg * 32, 0, 3);
		...
		if (searcherBarActive && useSearcherBar) {
			...
			for (int i = 0; i < typingSearcher.length() + 4; i++) {
				if (hasFrame) {
					screen.render(xSearcherBar + spaceWidth * i - leading, titleLoc.y - 8, 3 + bg * 32, 0, 3);
				}
		...
		for(int i = offset; i < (wrap ? offset + displayLength : Math.min(offset + displayLength, entries.size())); i++) {
			...
			if(!(entry instanceof BlankEntry)) {
				...
				if (searcherBarActive && useSearcherBar && p!=-1) {
					Font.draw(typingSearcher, screen, pos.x+(p*8), pos.y,  Color.YELLOW);
				};
	...
	private void renderFrame(Screen screen) {
		...
		for (int y = bounds.getTop(); y <= bottom; y += SpriteSheet.boxWidth) { // loop through the height of the bounds
			for (int x = bounds.getLeft(); x <= right; x += SpriteSheet.boxWidth) { // loop through the width of the bounds
				...
				screen.render(x, y, spriteoffset + bg * 32, mirrors, 3);
