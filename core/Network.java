...
public class Network extends Game {
	...
	public static void findLatestVersion(Action callback) {
		new Thread(() -> {
			...
			try {
				HttpResponse<JsonNode> response = Unirest.get("https://api.github.com/repos/pelletsstarPL/Minicraft-squared/releases").asJson();
		...
		if (latestVersion == null) {
			...
			findLatestVersion(() -> {
				if (latestVersion.version.compareTo(Game.VERSION) > 0) // Link new version
					System.out.println("VERSIONCHECK: Found newer version: Version " + latestVersion.releaseName + " Available! Download direct from \""+latestVersion.releaseUrl +"\". Can also be found with change log at \"https://github.com/pelletsstarPL/Minicraft-squared/releases\".");
