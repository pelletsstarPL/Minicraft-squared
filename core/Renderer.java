...
public class Renderer extends Game {
	...
	private static void renderLevel() {
		...
		if (currentLevel == 4) { // If the current level is sky one
			for (int y = 0; y < 28; y++)
				for (int x = 0; x < 48; x++) {
					screen.render(x * 8 - ((xScroll / 4) & 7), y * 8 - ((yScroll / 4) & 7), 2 + 25 * 32, 0, 1);
				}
		}
		if (currentLevel == 5) { // If the current level is second floor of obsidian dungeon - 1.7
			for (int y = 0; y < 28; y++)
				for (int x = 0; x < 48; x++) {
		...
		if (((currentLevel != 3 || Updater.tickCount < Updater.dayLength/4 || Updater.tickCount > Updater.dayLength/2) && !isMode("creative")) || player.potioneffects.containsKey(PotionType.Blind)) {
	...
	private static void renderGui() {
		...
		if (player.showpotioneffects && player.potioneffects.size() > 0) {
			...
			for (int i = 0; i < effects.length; i++) {
				...
				Font.drawTransparentBackground(pType + " (" + (pTime / 60) + ":" + (pTime % 60) + ")", screen, 180, 17 + i * Font.textHeight(), pType.dispColor);
			}
		}
		// This is the status icons, like health hearts, stamina bolts, and hunger "burgers".
		if (!isMode("creative")) {
			for (int i = 0; i < Player.maxStat; i++) {
				int poisoned=0;int hungered=0;int thirsted=0;int regen=0;int shield=0;int hardcore=0;
				...
				for(int j=0;j<10;j++) {
					...
					if (i < player.health && hpmod >= 2) {
						...
						if(player.getPotionEffects().containsKey(PotionType.Regen))screen.render(j * 8, Screen.h - 16, 0 + 10 * 32, 0, 3);
						if(player.getPotionEffects().containsKey(PotionType.Shield))screen.render(j * 8, Screen.h - 16, 0 + 8 * 32, 0, 3);
					} else if (hpmod == 1) {

						screen.render(j * 8, Screen.h - 16, (0+hardcore) + (3+poisoned) * 32, 0, 3); //half
						if(player.getPotionEffects().containsKey(PotionType.Regen))screen.render(j * 8, Screen.h - 16, 0 + 11 * 32, 0, 3);
						if(player.getPotionEffects().containsKey(PotionType.Shield))screen.render(j * 8, Screen.h - 16, 0 + 9 * 32, 0, 3);
						hpmod -= 1;
					}else if(hpmod<=0){
				...
				if (player.staminaRechargeDelay > 0) {
					...
					if (i < player.stamina) {
						if(player.getPotionEffects().containsKey(PotionType.Energy))screen.render(i * 8, Screen.h - 8, 1 + 5 * 32, 0, 3);
						else screen.render(i * 8, Screen.h - 8, 1 + 2 * 32, 0, 3);
					} else {
						if(player.getPotionEffects().containsKey(PotionType.Energy))screen.render(i * 8, Screen.h - 8, 1 + 6 * 32, 0, 3);
	...
	private static void renderDebugInfo() {
		...
		if (showinfo) { // Renders show debug info on the screen.
			...
			info.add("VERSION: 1.6.0");
			info.add(Initializer.fra + " fps");
			info.add("Day ticks: " + Updater.tickCount + " (" + Updater.getTime() + ")");
