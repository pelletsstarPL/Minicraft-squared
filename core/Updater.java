...
public class Updater extends Game {
	...
	public static float additionalChance = 0; // Measured in MULTIPLES OF NORMSPEED.
	public static boolean paused = true; // If the game is paused.
	public static boolean isbloody = false; // If the night is bloody
	...
	public static void tick() {
		...
		if(tickCount>0 && tickCount<8000){
			if(isbloody==true) {
				if(tickCount<1450)reduce=75-(tickCount/52);
				else reduce=47;
			}
			else{
				//System.out.println(reduce);
				if(tickCount<1450)reduce=(tickCount/16)-10 <0 ? 0 : (tickCount/16)-10;
				else{if(tickCount%24==0)reduce=(tickCount/16)-(tickCount-1450)/12 < 0 ? 0 : (tickCount/16)-(tickCount-1450)/12;}
			}
			if(reduce<0) reduce=0;
		}else if(tickCount>37800 && tickCount<=64800) {
			if(tickCount<58400)
				reduce = ((tickCount - 37800) / 45 < 0 ? 0 : (tickCount - 37800) / 45);
			else reduce=75;
		}
		if(tickCount==3500) {
			if (isbloody == false) {
				if ((bchance < 0.05 + additionalChance) && !pastDay1) {
					isbloody = true;
					additionalChance = 0;
				} else {
			...
			if (menu != null) {
				...
				if (debug && HAS_GUI) {
					...
					if (!ISONLINE || isValidServer()) {
						...
						if (input.getKey("1").clicked) changeTimeOfDay(Time.Dawn);
						if (input.getKey("2").clicked) changeTimeOfDay(Time.Morning);
						if (input.getKey("3").clicked) changeTimeOfDay(Time.Day);
						if (input.getKey("4").clicked) changeTimeOfDay(Time.Evening);
						if (input.getKey("5").clicked) changeTimeOfDay(Time.Night);
	...
	public static void setTime(int ticks) {
		if (ticks < Time.Dawn.tickTime) ticks = 0; // Error correct
		if (ticks < Time.Morning.tickTime) time = 0; // Morning
		else if (ticks < Time.Day.tickTime) time = 1; // Day
		else if (ticks < Time.Dusk.tickTime) time = 2; // Day
		else if (ticks < Time.Evening.tickTime) time = 3; // Day
		else if (ticks < Time.Night.tickTime) time = 4; // Evening
		else if (ticks < dayLength) time = 5; // Night
