...
public class ToolItem extends Item {
	...
	public String getDisplayName() {
		if (!type.noLevel) return (level==0 && (type.name()=="Bow" || type.name()=="Sword" || type.name()=="Claymore") ? " " + "Wood" + " " + type.name() : " " + Localization.getLocalized(LEVEL_NAMES[level]) + " " + Localization.getLocalized(type.toString()));
		...
		if (e instanceof Mob) {
			...
			if (type == ToolType.Axe || type == ToolType.Hammer) {
				if(LEVEL_NAMES[level]=="Candy"){
					...
					dmg =(int)( (level + 1) * 2.4 + random.nextInt(4 + level)+axebonus); // Wood axe damage: 2-5; gem axe damage: 10-13.
				}
