...
public abstract class Tile {
	public int extraSmallHbox = 0;
	public static int tickCount = 0; // A global tickCount used in the Lava & water tiles.
	public boolean canburn=false;
	public boolean isBurning=false;
	public boolean isSurface=true; //if tile is surface which means it is not fluid or pure air
	public int tntResistance = 0;
	...
	protected enum Material {
		...
		Stone2(ToolType.Pickaxe),
		Obsidian(ToolType.Pickaxe),
		ObsidianD(ToolType.Pickaxe),
		Unbreakable(ToolType.Hoe),
		Dungeon(ToolType.Pickaxe);
	...
	public boolean connectsToRock = false;
	public boolean connectsToMoss = false;
	public boolean connectsToSand = false;
	public boolean connectsToIce = false;
	public boolean connectsToGroundRock = false;
	public boolean connectsToObsidianBridgeSupport= false;
	...
	public boolean isTileWithin(int tileRadius, Entity other,int x,int y) {
		if (other.getLevel() == null) return false;
		//if ( != other.getLevel().depth) return false; // Obviously, if they are on different levels, they can't be next to each other.

		double distance = Math.abs(Math.hypot((x * 16) - other.x, y - other.y)); // Calculate the distance between the two entities, in entity coordinates.
