...
public class CDeadTreeTile extends Tile {
	private static Sprite sprite = new Sprite(2, 4, 2, 2, 1);

	protected CDeadTreeTile(String name) {
		super(name, sprite);
	}
	...
	public void hurt(Level level, int x, int y, int dmg) {
		...
		if (damage >= treeHealth) {
			...
			level.setTile(x, y, Tiles.get("Coarse dirt"));
		} else {
	...
	public void render(Screen screen, Level level, int x, int y) {
		Tiles.get("Coarse dirt").render(screen, level, x, y);
