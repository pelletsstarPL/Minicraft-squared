...
public class TreeTile extends Tile {
	...
	public enum TreeType{
		Oak("Oak",0,1,0,1,2,3,"Grass",new String[] {"Wood","Apple","Acorn","Stick"},20),
		Birch("Birch",17,18,18,19,20,21,"Grass",new String[] {"Wood","Catkin","Stick"},20),
		Conifer("Conifer",7,8,18,19,20,21,"Grass",new String[] {"Wood","Cone","Stick"},20),
		SkyConifer("Sky Conifer",19,20,18,19,20,21,"Skygrass",new String[] {"Wood","Cone","Stick"},20),
		SnowyConifer("Snowy Conifer",9,10,18,19,20,21,"Snow",new String[] {"Wood","Snow cone","Stick"},20),
		BigFungus("Fungus tree",5,6,18,19,20,21,"Moss",new String[] {"Fungus"},15),
		Sky("Sky Tree",11,12,18,19,20,21,"Skygrass",new String[] {"Wood","Stick"},20),
		MangroveWater("MangroveW",15,16,18,19,20,21,"Water",new String[] {"Wood","Stick"},20),
		Mangrove("Mangrove",13,14,18,19,20,21,"Moss",new String[] {"Wood","Stick"},20);
		private int sprite_x1,sprite_x2;
		private int sprite_y1,sprite_y2;
		private int sprite_y3,sprite_y4;
		private String baseTile,name;
		private String[] loot;
		private int health;

		TreeType(String name,int sprite_x1,int sprite_x2,int sprite_y1,int sprite_y2,int sprite_y3,int sprite_y4,String baseTile,String[] loot,int health){
			this.name=name;
	...
	protected TreeTile(TreeType type){
		super(type.name,(ConnectorSprite) null);
		this.type= type;
	...
	public void render(Screen screen, Level level, int x, int y) {
		...
		if(type.name()=="Mangrove" || type.name()=="MangroveW"){ //mangrove connectio so mangroves on water and mangroves on land can connect with themselves
			u = level.getTile(x, y - 1) == this || level.getTile(x, y - 1) == Tiles.get((type.name()=="Mangrove" ? "MangroveW" : "Mangrove"));
			d = level.getTile(x, y + 1) == this || level.getTile(x, y + 1) == Tiles.get((type.name()=="Mangrove" ? "MangroveW" : "Mangrove"));
			l = level.getTile(x - 1, y) == this || level.getTile(x - 1, y) == Tiles.get((type.name()=="Mangrove" ? "MangroveW" : "Mangrove"));
			r = level.getTile(x + 1, y) == this || level.getTile(x + 1, y) == Tiles.get((type.name()=="Mangrove" ? "MangroveW" : "Mangrove"));
			ur = level.getTile(x + 1, y - 1) == this || level.getTile(x + 1, y - 1) == Tiles.get((type.name()=="Mangrove" ? "MangroveW" : "Mangrove"));
			ul = level.getTile(x - 1, y - 1) == this || level.getTile(x - 1, y - 1) == Tiles.get((type.name()=="Mangrove" ? "MangroveW" : "Mangrove"));
			dl = level.getTile(x - 1, y + 1) == this || level.getTile(x - 1, y + 1) == Tiles.get((type.name()=="Mangrove" ? "MangroveW" : "Mangrove"));
			dr = level.getTile(x + 1, y + 1) == this || level.getTile(x + 1, y + 1) == Tiles.get((type.name()=="Mangrove" ? "MangroveW" : "Mangrove"));
		}
