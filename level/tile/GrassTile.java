...
public class GrassTile extends Tile {
	...
	public void render(Screen screen, Level level, int x, int y) {
		...
		if(level.depth==0)connectsToMoss = true;
	}
			...
			if (tool.type == ToolType.Hoe) {
				if (player.payStamina(4 - tool.level) && tool.payDurability()) {
					...
					if (random.nextInt(5) != 0) { // 80% chance to drop Wheat seeds
						double chance=Math.random(); //defines chance for beet seeds
						if(chance>0.2)
						level.dropItem(xt * 16 + 8, yt * 16 + 8, Items.get("Wheat Seeds"));
						else level.dropItem(xt * 16 + 8, yt * 16 + 8, Items.get("Beetroot Seeds"));
					}
					return true;
				}
			}
