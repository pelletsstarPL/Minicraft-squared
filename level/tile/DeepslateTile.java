...
public class DeepslateTile extends Tile {
	private ConnectorSprite sprite = new ConnectorSprite(DeepslateTile.class, new Sprite(24, 9, 3, 3, 1, 3), new Sprite(27, 11, 2, 2, 1, 3), new Sprite(27, 9, 2, 2, 1, 3));
	private boolean dropCoal = false;
	private int maxHealth = 85;

	private int damage;
	
	protected DeepslateTile(String name) {
	...
	public boolean interact(Level level, int xt, int yt, Player player, Item item, Direction attackDir) {
		if (item instanceof ToolItem) {
			...
			if (tool.type == ToolType.Pickaxe && player.payStamina(4 - tool.level) && tool.payDurability()) {
				...
				dropCoal = true;
				if(tool.level!=6) hurt(level, xt, yt, random.nextInt(10) + (tool.level) * 5 + 10);
				else hurt(level, xt, yt, random.nextInt(10) + (2) * 5 + 2);
				return true;
			}
		}
		return false;
