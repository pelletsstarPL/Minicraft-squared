...
public class RosesTile extends Tile {
	private static final Sprite flowerSprite = new Sprite(5, 6, 1);
	
	protected RosesTile(String name) {
	...
	public boolean hurt(Level level, int x, int y, Mob source, int dmg, Direction attackDir) {
		level.dropItem(x *16 + 8, y * 16 + 8, 1, 1, Items.get("Rose"));
		double chance=Math.random();
		if(chance<0.2)level.dropItem(x *16 + 8, y * 16 + 8, 1, 1, Items.get("Rose"));
		level.setTile(x, y, Tiles.get("Grass"));
		return true;
	}
}
