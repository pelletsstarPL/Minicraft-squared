...
public class SmallFlowerTile extends Tile {
	private static final Sprite flowerSprite = new Sprite(4, 8, 1);
	
	protected SmallFlowerTile(String name) {
		...
		if (item instanceof ToolItem) {
			...
			if (tool.type == ToolType.Shovel) {
				if (player.payStamina(2 - tool.level) && tool.payDurability()) {
					...
					level.dropItem(x * 16 + 8, y * 16 + 8, Items.get("Small flower"));
					return true;
				}
			}
		}
	...
	public boolean hurt(Level level, int x, int y, Mob source, int dmg, Direction attackDir) {
		level.dropItem(x *16 + 8, y * 16 + 8, 1, 1, Items.get("Small flower"));
		double chance=Math.random();
		if(chance<0.2)level.dropItem(x *16 + 8, y * 16 + 8, 1, 1, Items.get("Small flower"));
		level.setTile(x, y, Tiles.get("Grass"));
		return true;
	}
}
