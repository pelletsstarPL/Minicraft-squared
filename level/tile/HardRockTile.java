...
public class HardRockTile extends Tile {
	...
	private static ConnectorSprite sprite = new ConnectorSprite(HardRockTile.class, new Sprite(18, 9, 3, 3, 1, 3), new Sprite(21, 10, 2, 2, 1, 3),new Sprite(26, 30, 2, 2, 1));
	...
	public boolean interact(Level level, int xt, int yt, Player player, Item item, Direction attackDir) {
		...
		if (item instanceof ToolItem) {
			...
			if (tool.type == ToolType.Pickaxe && (tool.level == 4 || tool.level==5)) {
				if (player.payStamina(3) && tool.payDurability()) {
					hurt(level, xt, yt, random.nextInt(10) + (tool.level) * 5 + 10);
					return true;
				}
			} else {
				Game.notifications.add("Gem Pickaxe or stronger Required.");
			}
		}
		return false;
	}
