...
public class WallTile extends Tile {
		...
		if (item instanceof ToolItem) {
			...
			if (tool.type == type.getRequiredTool()) {
				...
				if(type==Material.Obsidian){
					...
					}else if (level.depth != -3 ||(type != Material.Obsidian && type!=Material.ObsidianD) || AirWizardPhase4.beaten) {
					if (player.payStamina(4 - tool.level) && tool.payDurability()) {
						hurt(level, xt, yt, random.nextInt(10) + (tool.level) * 5 + 10);
						return true;
					}else if(type==Material.ObsidianD){
						if((tool.level>1 && tool.level!=6) && AirWizardPhase4.beaten){
							if (player.payStamina(4 - tool.level) && tool.payDurability()) {
								hurt(level, xt, yt, random.nextInt(10) + (tool.level) * 5 + 10);
								return true;
							}
	...
	public void hurt(Level level, int x, int y, int dmg) {
		...
		if (damage >= sbwHealth) {
			...
			if(type!=Material.Dungeon && type!=Material.ObsidianD)level.dropItem(x * 16 + 8, y * 16 + 8, 1, 3 - type.ordinal(), Items.get(itemName));
			else if(type==Material.Dungeon)level.dropItem(x * 16 + 8, y * 16 + 8, 1, 2, Items.get("Stone Brick"));
