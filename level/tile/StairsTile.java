...
public class StairsTile extends Tile {
	...
	public void render(Screen screen, Level level, int x, int y) {
		...
		switch (level.depth){
			case 1:Tiles.get(level.realm.contains("overworld") ? "Cloud" : "Obsidian bridge support").render(screen,level,x,y);break; //Sky
		...
		for(Tile t: areaTiles){
			...
			if(t == Tiles.get("Ground rock")){
				connectsToGroundRock = true;
				Tiles.get("Ground rock").render(screen, level, x, y);
			}
		}
