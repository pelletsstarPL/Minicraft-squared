...
public class LevelGen {
	...
	private static short[][] createAndValidateTopMap(int w, int h) {
		...
		do {
			...
			if (count[Tiles.get("Stairs Down").id & 0xff] == 0) continue; // size 128 = 6 stairs min
	...
	private static short[][] createAndValidateUndergroundMap(int w, int h, int depth) {
		...
		do {
			...
			if(depth==2) if (count[(Tiles.get("iron Ore").id & 0xff)] < 100) continue;
	...
	private static short[][] createTopMap(int w, int h) { // Create surface map
		...
		for (int y = 0; y < h; y++) {
			for (int x = 0; x < w; x++) {
				...
				switch ((String) Settings.get("Type")) {
					...
					case "Irregular":
						if (val < -0.5 && mval < -0.5) {
							if (Settings.get("Theme").equals("Tundra")) {
								map[i] = Tiles.get("ice").id;
							}
							if (Settings.get("Theme").equals("Hell")) {
								map[i] = Tiles.get("lava").id;
							}
							if (!Settings.get("Theme").equals("Hell") && !Settings.get("Theme").equals("Tundra")) {
								map[i] = Tiles.get("water").id;
							}
						} else if (val > 0.5 && mval < -1.5) {
							map[i] = Tiles.get("rock").id;
		...
		if (Settings.get("Theme").equals("Taiga")) {
			for (int i = 0; i < w * h / 200; i++) {
				int x = random.nextInt(w);
				int y = random.nextInt(h);
				for (int j = 0; j < 200; j++) {
					...
					if (xx >= 0 && yy >= 0 && xx < w && yy < h) {
						if (map[xx + yy * w] == Tiles.get("grass").id || map[xx + yy * w] == Tiles.get("tall grass").id || map[xx + yy * w] == Tiles.get("Grass Small stones").id) {
							...
							if(chance>0.003) {
								...
								if(chance<=0.003 && chance>=0.001) map[xx + yy * w] = Tiles.get("tree").id;
						...
						for (int yy = yo - 1; yy <= yo + 1; yy++) {
							for (int xx = xo - 1; xx <= xo + 1; xx++) {
								if (xx >= 0 && yy >= 0 && xx < w && yy < h) {
									...
									if(xx>10 && xx<w-10 && yy>10 && yy<h-10)
									if(map[xx + yy * w] == Tiles.get("water").id)map[xx + yy * w] = Tiles.get("ice").id;
								}
							}
						}
					}
				...
				for (int k = 0; k < 100; k++) {
					...
					for (int j = 0; j < 200; j++) {
						...
						for (int yy = yo - 1; yy <= yo + 1; yy++) {
							for (int xx = xo - 1; xx <= xo + 1; xx++) {
								if (xx >= 0 && yy >= 0 && xx < w && yy < h) {
									if (map[xx + yy * w] == Tiles.get("grass").id || map[xx + yy * w] == Tiles.get("tall grass").id || map[xx + yy * w] == Tiles.get("Grass Small stones").id || map[xx + yy * w] == Tiles.get("Conifer").id || map[xx + yy * w] == Tiles.get("Tree").id || map[xx + yy * w] == Tiles.get("Birch").id) {
										map[xx + yy * w] = Tiles.get("snow").id;
									}
									if(xx>10 && xx<w-10 && yy>10 && yy<h-10)
									if(map[xx+yy*w] == Tiles.get("water").id && ((map[(xx-1)+yy*w] == Tiles.get("water").id || (map[(xx+1)+yy*w] == Tiles.get("water").id))))map[xx + yy * w] = Tiles.get("Ice").id;
								}
							}
						}
					}
				...
				for (int j = 0; j < 60; j++) {
					...
					if (xx >= 0 && yy >= 0 && xx < w && yy < h) {
						if (map[xx + yy * w] == Tiles.get("snow").id) {
							map[xx + yy * w] = Tiles.get("snowy conifer").id;
						}
					}
				}
			}
		...
		if (!Settings.get("Theme").equals("Tundra")) {
			for (int i = 0; i < w * h / 200; i++) {
				...
				int y = random.nextInt(h);
				for (int j = 0; j < 60; j++) {
					int xx = x + random.nextInt(15) - random.nextInt(11);
					int yy = y + random.nextInt(15) - random.nextInt(11);
					if (xx >= 0 && yy >= 0 && xx < w && yy < h) {
						if (map[xx + yy * w] == Tiles.get("snow").id) {
							map[xx + yy * w] = Tiles.get("snowy conifer").id;
						}
		...
		for (int y = 0; y < h; y++) {
			for (int x = 0; x < w; x++) {
				...
				if (val > -1 && wval < -1 + (depth) / 2 * 3) {
					if (depth == 4) {
						if(val>-1 && val<-0.82)map[i] = Tiles.get("dirt").id;
						else if(val>=-0.82 && val<-0.8)map[i] = Tiles.get("Raw Obsidian").id;
						...
						if(i>10 && i<(w*h)-10) {
							...
							if(val>-0.75 && val <-0.57)map[i] = Tiles.get("Ground rock").id;
						}
					}else if (depth == 2) {
						if(val >-0.8 && val <-0.64) {
						...
						if(val > 2.5) {
							map[i] = Tiles.get("water").id;
							if (map[i - 1] == Tiles.get("Moss").id || map[i - 1] == Tiles.get("Azalea").id)
								map[i - 1] = Tiles.get("dirt").id;
							if (map[i - 2] == Tiles.get("Moss").id || map[i - 2] == Tiles.get("Azalea").id)
								map[i - 2] = Tiles.get("dirt").id;//right side shores
							int reedGen = (int) Math.round(Math.random() * 3);
							double chance = Math.random();
							if (map[i - reedGen] == Tiles.get("water").id && chance < 0.007)
								map[i - reedGen] = Tiles.get("reed").id; //to make sugar obtainable in hell worlds
							int spikeGen = (int) Math.round(Math.random() * 4);
							//int spikeGenLand=(int) Math.round(Math.random()*9);
							double chance2 = Math.random();
							if (map[i - spikeGen] == Tiles.get("water").id && chance2 < 0.01)
								map[i - spikeGen] = Tiles.get("Spiky stone").id; //to make sugar obtainable in hell worlds
						}else if(val>2.1 && val<=2.5){
							map[i] = Tiles.get("Ground rock").id;
						}else{
							double chance2 = Math.random();
							if(chance2<0.03)map[i] = Tiles.get("reed").id;
					...
					if(depth==4){
						...
						if (x >= 0 && y >= 0 && x < w && y < h) {
							if (map[i] == Tiles.get("Moss").id && chance < 0.34) {
								...
								else if (chance > 0.12 && chance < 0.26) map[i] = Tiles.get("Azalea").id;
								else if(chance>0.08 && chance<=0.12)map[i] = Tiles.get("Bramble").id;
				...
				if (val < -0.27) {
					map[i] = Tiles.get("Infinite Fall").id;
						...
						if(val>=-0.27 && val<0.2) {
							...
							if (Math.random() < 0.33) map[i] = Tiles.get("conifer").id;
	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			short[][] fullmap = LevelGen.createAndValidateMap(w, h, -2);
			...
			for (int y = 0; y < h; y++) {
				for (int x = 0; x < w; x++) {
					...
					if (map[i] == Tiles.get("cloud").id) pixels[i] = 0x939393;
