	...
	private static byte[][] createAndValidateUndergroundMap(int w, int h, int depth) {
		...
		do {
			...
			if(depth!=2) {
				...
				if (count[Tiles.get("Moss").id & 0xff] < 100) continue;
			}
				...
				switch ((String) Settings.get("Type")) {
					case "Island":
						...
						if (val < -0.5) {
							...
							else if(chance<=0.3 && chance>0.03)map[i] = Tiles.get("tall grass").id;
							else map[i] = Tiles.get("Grass small stones").id;
						}
					...
					case "Mountain":
						...
						} else if(mval>=1.5 && mval<1.62){
							double chance=Math.random();
							if(chance>0.013) map[i] = Tiles.get("Spiky stone-L").id;
			...
			for (int i = 0; i < w * h / 400; i++) {
				...
				for (int j = 0; j < 200; j++) {
					...
					if (xx >= 0 && yy >= 0 && xx < w && yy < h) {
						if (map[xx + yy * w] == Tiles.get("grass").id) {
							double typetree=Math.round(Math.random());
							double smalltree=Math.random();
							if(typetree==0) {
								if(smalltree>0.1)map[xx + yy * w] = Tiles.get("tree").id;
								else map[xx + yy * w] = Tiles.get("small tree").id;
							}else{
								map[xx + yy * w] = Tiles.get("birch").id;
							}
						}
			...
			for (int j = 0; j < 30; j++) {
				...
				if (xx >= 0 && yy >= 0 && xx < w && yy < h) {
					if (map[xx + yy * w] == Tiles.get("grass").id || map[xx + yy * w] == Tiles.get("Tall grass").id) {
						double stonech=Math.random();
					/*	if(stonech<0.9) {
							if (map[(xx + 1) + (yy) * w] == Tiles.get("Rock").id)
								map[(xx + 1) + (yy) * w] = Tiles.get("Grass Small stones").id;
							if (map[(xx - 1) + (yy) * w] == Tiles.get("Rock").id)
								map[(xx - 1) + (yy) * w] = Tiles.get("Grass Small stones").id;
							if (map[(xx) + (yy + 1) * w] == Tiles.get("Rock").id)
								map[(xx) + (yy + 1) * w] = Tiles.get("Grass Small stones").id;
							if (map[(xx) + (yy - 1) * w] == Tiles.get("Rock").id)
								map[(xx) + (yy - 1) * w] = Tiles.get("Grass Small stones").id;
							if (map[(xx + 1) + (yy - 1) * w] == Tiles.get("Rock").id)
								map[(xx + 1) + (yy) * w] = Tiles.get("Grass Small stones").id;
							if (map[(xx - 1) + (yy + 1) * w] == Tiles.get("Rock").id)
								map[(xx - 1) + (yy) * w] = Tiles.get("Grass Small stones").id;
							if (map[(xx - 1) + (yy + 1) * w] == Tiles.get("Rock").id)
						...
						double flotype = Math.round(Math.random() * 5);
						double chance=Math.random();
						if (flotype == 0) {
							map[xx + yy * w] = Tiles.get("flower").id;
						} else if (flotype == 1){ map[xx + yy * w] = Tiles.get("rose").id;
					} else if (flotype == 2){ map[xx + yy * w] = Tiles.get("small flower").id;
				} else if (flotype == 3){ map[xx + yy * w] = Tiles.get("small rose").id;
			}else if(flotype==4){ map[xx + yy * w] = Tiles.get("sunflower").id;
						}
						else if(flotype==5){ double chance1=Math.random();
	...
	private static byte[][] createDungeon(int w, int h) {
		...
		for (int y = 0; y < h; y++) {
			for (int x = 0; x < w; x++) {
				...
				Structure.dungeonOre.draw(map, 20, 20, w);
			}
		}
		...
		for (int i = 0; i < w * h / 450; i++) {
			...
			Structure.lavaPool.draw(map, x, y, w);
					...
					if(x<6 || y<6) Structure.lavaFountain.draw(map, x+11, y+11, w);
					else if(x>(w-6) || y>(h-6))Structure.lavaFountain.draw(map, x-11, y-11, w);
		}
		oreLoop:
		for (int i = 0; i < w * h / 450; i++) {
			int x = random.nextInt(w - 2) + 1;
			...
			for (int x = 0; x < w; x++) {
				...
				if (val > -1 && wval < -1 + (depth) / 2 * 3) {
					if (depth == 3){
						map[i] = Tiles.get("lava").id;
						double chance=Math.random();
						if(map[i-1] == Tiles.get("dirt").id && chance<0.4)map[i-1] = Tiles.get("Raw Obsidian").id; //obsidian shores
						if(map[i-2] == Tiles.get("dirt").id && chance<0.2)map[i-2] = Tiles.get("Raw Obsidian").id;
						if(map[i-1] == Tiles.get("Deepslate").id && chance<0.4)map[i-1] = Tiles.get("Small stones").id;
						if(map[i-2] == Tiles.get("Deepslate").id && chance<0.2)map[i-2] = Tiles.get("Small stones").id;
						if(map[i+1] == Tiles.get("Deepslate").id && chance<0.4)map[i-1] = Tiles.get("Small stones").id;
						if(map[i+2] == Tiles.get("Deepslate").id && chance<0.2)map[i-2] = Tiles.get("Small stones").id;
						int spikeGen=(int) Math.round(Math.random()*3);
						if(map[i-spikeGen]==Tiles.get("lava").id && chance<0.01)map[i-spikeGen] = Tiles.get("Deepslate spiky stone").id;
					}
					else if (depth == 1) {
						...
						map[i] = Tiles.get("water").id;
						if(map[i-1] == Tiles.get("Moss").id || map[i-1] == Tiles.get("Azalea").id)map[i-1] = Tiles.get("dirt").id;
						if(map[i-2] == Tiles.get("Moss").id || map[i-2] == Tiles.get("Azalea").id)map[i-2] = Tiles.get("dirt").id;//right side shores
						int reedGen=(int) Math.round(Math.random()*3);
						double chance=Math.random();
						if(map[i-reedGen]==Tiles.get("water").id && chance<0.007)map[i-reedGen] = Tiles.get("reed").id; //to make sugar obtainable in hell worlds
						int spikeGen=(int) Math.round(Math.random()*4);
						//int spikeGenLand=(int) Math.round(Math.random()*9);
						double chance2=Math.random();
						if(map[i-spikeGen]==Tiles.get("water").id && chance2<0.01)map[i-spikeGen] = Tiles.get("Spiky stone").id; //to make sugar obtainable in hell worlds
					...
					if(depth==2){
						map[i] = Tiles.get("Moss").id;
						double chance=Math.random();
						if (x >= 0 && y >= 0 && x < w && y < h) {
							if (map[i] == Tiles.get("Moss").id && chance<0.34) {
								int col = random.nextInt(4);
								if(chance>=0.26) map[i] = Tiles.get("Big Fungus").id;
								else if(chance>0.08 && chance<0.26) map[i] = Tiles.get("Azalea").id;
								else if(chance>0.04 && chance<=0.08) map[i]=Tiles.get("Dirt").id;
								else if(chance>0.01 && chance<=0.04) map[i] = Tiles.get("Water").id;
								else map[i]=Tiles.get("Spiky stone-L").id;
								data[i] = (byte) (col + random.nextInt(4) * 16); // Data determines which way the Azalea faces
							}
						}
					}
					else {
					...
					if(depth==3) {
						double chance=Math.random();
						if(chance<0.1 && (map[i+1] == Tiles.get("dirt").id || map[i-1] == Tiles.get("dirt").id || map[i+1] == Tiles.get("lava").id || map[i-1] == Tiles.get("lava").id)) map[i] = Tiles.get("Deepslate spiky stone-L").id;
				...
				if(depth<3) {
					for (int j = 0; j < 30; j++) {
						...
						if (xx >= r && yy >= r && xx < w - r && yy < h - r) {
							if (map[xx + yy * w] == Tiles.get("rock").id) {
								double chance=Math.random();
								if(chance>0.2) map[xx + yy * w] = (byte) ((Tiles.get("iron Ore").id & 0xff) + depth - 1);
								else map[xx + yy * w] = (byte) ((Tiles.get("ironnf Ore").id & 0xff) + depth - 1);
							}
						}
					}
					for (int j = 0; j < 10; j++) {
					...
					for (int j = 0; j < 30; j++) {
						...
						int yy = y + random.nextInt(5) - random.nextInt(5);
						if (xx >= r && yy >= r && xx < w - r && yy < h - r) {
							if (map[xx + yy * w] == Tiles.get("deepslate").id) {
								if(depth==3) {
									double chance=Math.random();
									if(chance>0.04) {
										if (chance > 0.2) map[xx + yy * w] = (byte) (Tiles.get(gem).id & 0xff);
										else map[xx + yy * w] = (byte) (Tiles.get(gemnf).id & 0xff);
									}
									else map[xx + yy * w] = (byte) (Tiles.get("Lapis").id & 0xff);
								}
							}
						}
					}
					for (int j = 0; j < 10; j++) {
						...
						if (xx >= r && yy >= r && xx < w - r && yy < h - r) {
							if (map[xx + yy * w] == Tiles.get("deepslate").id) {
								if(depth!=3) map[xx + yy * w] = (byte) (Tiles.get("Lapis").id & 0xff);
								else map[xx + yy * w] =(byte) (Tiles.get("Obsidian Ore").id & 0xff);
							}
						}
					}
				}
			...
			for (int i = 0; i < w * h / 380; i++) {
				for (int j = 0; j < 10; j++) {
					if (xx < w - r && yy < h - r) {
						...
						map[xx + yy * w] = (byte) (Tiles.get("Obsidian Stairs Down").id & 0xff);
					}
				}
			}
		}
			...
			for (int x = 0; x < w; x++) {
				...
				if (val < -0.25) {
					map[i] = Tiles.get("Infinite Fall").id;
						...
						if(val>0-25 && val<0.5)	map[i] = Tiles.get("cloud").id;
						else if(val>=0.5 && val<1)map[i] = Tiles.get("Skygrass").id;
						else  map[i] = Tiles.get("Skyrock").id;
					}
				}
			}
		...
		while (true) {
			...
			for (int y = 0; y < h; y++) {
				for (int x = 0; x < w; x++) {
					...
					if (map[i] == Tiles.get("deepslate").id) pixels[i] = 0x707070;
