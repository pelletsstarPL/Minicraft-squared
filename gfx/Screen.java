    ...
    public void overlay(Screen screen2, int currentLevel, int xa, int ya) {
		...
		if (currentLevel >= 3 && currentLevel < 5) {
			...
			switch (Updater.getTime()) {
				case Morning: tintFactor = Updater.pastDay1 ? (1-relTime) * MAXDARK : 0; break;//ends at 16400
				case Day: tintFactor = 0; break; //ends at 32768
            ...
            for (int x = 0; x < w; x++) { // loop through width of screen
				if (oPixels[i] / 10 <= dither[((x + xa) & 3) + ((y + ya) & 3) * 4]) {
					...
					if (currentLevel < 3) { // if in caves...
						...
						if(Game.player.potioneffects.containsKey(PotionType.Blind))pixels[i] = 0; //if blindness potion is active make everything pitch black even on overworld
						else
