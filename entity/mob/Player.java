...
public class Player extends Mob implements ItemHolder, ClientTickable {
	...
	private int thirstStamCnt, stamThirstTicks; // Tiers of thirst penalties before losing a burger.
	private static final int maxThirstTicks = 260; // The cutoff value for stamThirstTicks
	private static final int[] maxThirstStams = {10, 7, 6}; // ThirstStamCnt required to lose a burger.
	private static final int[] thirstTickCount = {110, 30, 10}; // Ticks before decrementing stamThirstTicks.
	private static final int[] thirstStepCount = {8, 3, 1}; // Steps before decrementing stamThirstTicks.
	private static final int[] minDehydrationHealth = {5, 3, 0}; // Min hearts required for thirst to hurt you.
	...
	public void tick() {
		...
		if (Game.getMenu() == null && !Bed.inBed(this)) {
			...
			if ((vec.x != 0 || vec.y != 0) && (staminaRechargeDelay % 2 == 0 || isSwimming()) && !Updater.saving) {
				double spd = moveSpeed * (potioneffects.containsKey(PotionType.Speed) ? 1.5 : 1);
	...
	protected void attack() {
		...
		if (activeItem != null) {
			...
			/*//get dirty water
			if(attackItem instanceof BottleItem && Game.levels[level.depth].getTile(x >> 4, y >> 4).name=="Water"){
				inventory.removeItem(Items.get("Bottle"));
				inventory.add(Items.get("Dirty water"));
			}*/
