	...
	public void tick() {
		...
		if(!Bed.inBed(this)) {
			...
			if (stepCount >= Player.hungerStepCount[diffIdx]) {
				...
				stamThirstTicks--; // thirst due to exercise.
				if (potioneffects.containsKey(PotionType.Hunger)){
					stamHungerTicks-=1;
		...
		if(!Bed.inBed(this)) {
			...
			if (Updater.tickCount % Player.thirstTickCount[diffIdx] == 0) {
				stamThirstTicks--; // thirst due to time.
				if (potioneffects.containsKey(PotionType.Thirst)) {
					stamThirstTicks -= 2;
	...
	public void render(Screen screen) {
		...
		if (Updater.tickCount / 10 % 2 == 0) {
			...
			if (level.getTile(x / 16, y / 16) == Tiles.get("water") ) {
				if(!Updater.paused)level.add(new SplashParticle(x - 8 + randX, y - 8 + randY));
			}
			else if (level.getTile(x / 16, y / 16) == Tiles.get("lava")) {
				if(!Updater.paused)level.add(new BurnParticle(x - 8 + randX, y - 8 + randY));
			}
		}
		if (isSwimming()) {
