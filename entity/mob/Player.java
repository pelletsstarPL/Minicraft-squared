...
public class Player extends Mob implements ItemHolder, ClientTickable {
	...
	private static final int[] hungerTickCount = {120, 32, 16}; // Ticks before decrementing stamHungerTicks.
	private static final int[] hungerStepCount = {8, 3, 2}; // Steps before decrementing stamHungerTicks.
	...
	private int thirstStamCnt, stamThirstTicks; // Tiers of thirst penalties before losing a droplet.
	...
	private static final int[] thirstTickCount = {110, 32, 16}; // Ticks before decrementing stamThirstTicks.
	private static final int[] thirstStepCount = {8, 3, 2}; // Steps before decrementing stamThirstTicks.
	...
	public void tick() {
		...
		if(isSprinting==true && stamina!=0) {
			if (sprintstaminatakedelay > (potioneffects.containsKey(PotionType.Fatigue) ? 4 : 6)){
		...
		if(!Bed.inBed(this)) {
			if (hungerChargeDelay > 0) { // If the hunger is recharging health...
				stamHungerTicks -= 2+diffIdx/(potioneffects.containsKey(PotionType.WellFed) ? 1.25 : 1); // Penalize the hunger
			...
			if (Updater.tickCount % Player.hungerTickCount[diffIdx] == 0) {
				stamHungerTicks-=(potioneffects.containsKey(PotionType.WellFed) ? 0.25 : 1); // hunger due to time.
			...
			if (stepCount >= Player.hungerStepCount[diffIdx]) {
				stamHungerTicks-=(potioneffects.containsKey(PotionType.WellFed) ? 0.75 : 1); // hunger due to exercise.
				stamThirstTicks-=(potioneffects.containsKey(PotionType.WellHydrated) ? 0.8 : 1); // thirst due to exercise.
				if (potioneffects.containsKey(PotionType.Hunger)){
					stamHungerTicks-=1;
		...
		if(!Bed.inBed(this)) {
			if (thirstChargeDelay > 0) { // If the thirst is recharging health...
				stamThirstTicks -= 2+diffIdx; // Penalize the thirst
			...
			if (Updater.tickCount % Player.thirstTickCount[diffIdx] == 0) {
				stamThirstTicks-=(potioneffects.containsKey(PotionType.WellHydrated) ? 0.33 : 1); // thirst due to time.
				if (potioneffects.containsKey(PotionType.Thirst)) {
					stamThirstTicks -= 2;
	...
	private boolean hurt(Rectangle area) {
		...
		for (Entity e : entities) {
			if (e != this && e instanceof Mob) {
				...
				((Mob) e).hurt(this, dmg, attackDir,crit); //postponed for 2.5
			}
	...
	public void render(Screen screen) {
		...
		if (isSwimming()) {
			...
			if (level.getTile(x / 16, y / 16) == Tiles.get("water") || level.getTile(x / 16, y / 16) == Tiles.get("Reed")) {
				if(!Updater.paused)screen.render(xo + 0, yo + 3, 5 + 2 * 32, 0, 3); // Render the water graphic
				if(!Updater.paused)screen.render(xo + 8, yo + 3, 5 + 2 * 32, 1, 3); // Render the mirrored water graphic to the right.
			} else if (level.getTile(x / 16, y / 16) == Tiles.get("lava")) {
				if(!Updater.paused)screen.render(xo + 0, yo + 3, 6 + 2 * 32, 1, 3); // Render the lava graphic
				if(!Updater.paused)screen.render(xo + 8, yo + 3, 6 + 2 * 32, 0, 3); // Render the mirrored lava graphic to the right.
		...
		if(isSprinting==true && animtik%6<3){
			if(!Updater.paused)screen.render(xo + 0, yo + 7, 6 + 4 * 32, 1, 3); // Render the sprinting frame 1 graphic
			if(!Updater.paused)screen.render(xo + 8, yo + 7, 6 + 4 * 32, 0, 3); // Render the mirrored
		}else if(isSprinting==true && animtik%6>=3){
			if(!Updater.paused)screen.render(xo + 0, yo + 7, 5 + 4 * 32, 0, 3); // Render the sprinting frame 2 graphic
			if(!Updater.paused)screen.render(xo + 8, yo + 7, 5 + 4 * 32, 1, 3); // Render the mirrored
		}
