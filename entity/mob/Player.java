...
public class Player extends Mob implements ItemHolder, ClientTickable {
	...
	private int isSprinting=0; //0 or 1
	...
	public void tick() {
		animtik++;
		if((hunger<6 || thirst<6) && isSprinting==1)isSprinting=0;
		...
		if(input.getKey("sprint").clicked && sprintdelay == 0 && isSprinting == 0 && !isSwimming() && thirst>6 && hunger>6) {
			sprintdelay = 10;
			isSprinting = 1;
			Sound.sprint.play();
		}else if(input.getKey("sprint").clicked && sprintdelay == 0 && isSprinting == 1) {
			sprintdelay = 400;
			isSprinting = 0;
		}
		if(isSprinting==1 && stamina!=0) {
		...
		}else if(stamina==0) {isSprinting=0;sprintdelay = 400;}
		...
		if (staminaRechargeDelay == 0) {
			...
			if(isSwimming() && isSprinting==1){isSprinting=0;sprintdelay = 400;}
		...
		if (Game.getMenu() == null && !Bed.inBed(this)) {
			...
			if ((vec.x != 0 || vec.y != 0) && (staminaRechargeDelay % 2 == 0 || isSwimming()) && !Updater.saving) {
				double spd=(moveSpeed) * (potioneffects.containsKey(PotionType.Speed) ? 1.5 : 1)+(isSprinting==1 ? 1 : 0);
	...
	public void render(Screen screen) {
		...
		if (Updater.tickCount / 10 % 2 == 0) {
			...
			if (level.getTile(x / 16, y / 16) == Tiles.get("water") ) {
				...
				level.add(new BurnParticle(x - 8 + randX, y - 8 + randY));
			}
		}
		if (isSwimming()) {
		...
		if(isSprinting==1 && animtik%6<3){
			screen.render(xo + 0, yo + 7, 6 + 4 * 32, 1, 3); // Render the sprinting frame 1 graphic
			screen.render(xo + 8, yo + 7, 6 + 4 * 32, 0, 3); // Render the mirrored
		}else if(isSprinting==1 && animtik%6>=3){
			screen.render(xo + 0, yo + 7, 5 + 4 * 32, 0, 3); // Render the sprinting frame 2 graphic
			screen.render(xo + 8, yo + 7, 5 + 4 * 32, 1, 3); // Render the mirrored
		}
	...
	protected void doHurt(int damage, Direction attackDir) {
		...
		if (fullPlayer) {
			if (curArmor == null) { // No armor
				...
				if(Math.random()<0.025 && hunger>0){
					hunger--;
					Game.player.potioneffects.put(PotionType.Time, 400);
				}
