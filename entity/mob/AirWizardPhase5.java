...
import minicraft.entity.Spark5;
...
public class AirWizardPhase5 extends EnemyMob {
	 ...
	 */
	public AirWizardPhase5(int lvl) {
		this(lvl > 1);
	}
	 ...
	 */
	public AirWizardPhase5(boolean secondform) {
	...
	public void tick() {
		...
		if (attackTime > 0) {
			xmov = ymov = 0;
			attackTime *= secondform ? 0.995 : 0.99; // attackTime will decrease by 7% every time and by 3% for second tier phase 1
			...
			level.add(new Spark5(this, Math.cos(dir) * speed, Math.sin(dir) * speed)); // Adds a spark entity with the cosine and sine of dir times speed.
	...
	public void render(Screen screen) {
		...
		if (AirWizard.invulnerability > 0 && renderTime % 3 == 0) {
			...
			lvlSprites[lvl-1] = (secondform ? MobSprite.compileMobSpriteAnimations(0, 18) : MobSprite.compileMobSpriteAnimations(0, 16));
		}
	...
	public void die() {
		...
		if(!secondform) {
			...
			if (!(boolean)Settings.get("unlockedskin")) {
				...
				Updater.notifyAll("A costume and a book lies on the ground...", -400);
