...
public class NightWizard extends EnemyMob {
    ...
    static {
        ...
        for (int i = 0; i < 1; i++) {
            MobSprite[][] list  = MobSprite.compileMobSpriteAnimations(8, 32 + (i * 2));
            sprites[i] = list;
            sleepSprite = MobSprite.compileMobSpriteAnimations(0, 34+ (i*2));
        }
    }
    ...
    public NightWizard(int lvl) {
        super(lvl, sprites, 3500, false, 16 * 9, -1, 10, 50);
    ...
    public void tick() {
        ...
        if (Updater.getTime() == Updater.Time.Night) {
            if (Updater.tickCount % 2100 == 0) {
                ...
                Game.notifications.add("This is your last night.");
                for (int i = 0; i < 301; i++) {
                    if (i % 100 == 0) {
                        attackTime = 200;
                ...
                if (level.mobCount + 60 < level.maxMobCount) {
                    ...
                    for (int i = 0; i <= 60; i++) {
                        ...
                        while(level.getTile(nx,ny) == Tiles.get("Path") || level.getTile(nx,ny) == Tiles.get("Dirt") || level.getTile(nx,ny) == Tiles.get("Grass") || level.getTile(nx,ny) == Tiles.get("Desert grass") || level.getTile(nx,ny) == Tiles.get("Tall grass") || level.getTile(nx,ny) == Tiles.get("Snow") || level.getTile(nx,ny) == Tiles.get("Wood Planks") || level.getTile(nx,ny) == Tiles.get("Stone bricks") || level.getTile(nx,ny) == Tiles.get("Flower")){
                            nx = random.nextInt(level.w) * 16 + 8; ny = random.nextInt(level.h) * 16 + 8; //reroll
                        }
                        if (rnd < 2) level.add((new Wraith((revenge + 2) > 4 ? 4 : (revenge + 2))), nx, ny);
                        else if (rnd >= 2 && rnd <= 40)
                            level.add((new Slime(1 + (revenge + 2) > 4 ? 4 : (revenge + 2))), nx, ny);
                        else if (rnd <= 75) level.add((new Zombie((revenge + 2) > 4 ? 4 : (revenge + 2))), nx, ny);
                        else if (rnd >= 85) level.add((new Skeleton((revenge + 2) > 4 ? 4 : (revenge + 2))), nx, ny);
                    }
                }
            }
    ...
    public void render(Screen screen) {
        ...
        if (Updater.getTime()!= Updater.Time.Night) {
            ...
            lvlSprites[lvl-1] = (MobSprite.compileMobSpriteAnimations(8, 34));
        }else{
            lvlSprites[lvl-1] = (MobSprite.compileMobSpriteAnimations(8, 32));
        }
    ...
    public void die() {
        ...
        if (players.length > 0) { // If the player is still here
            for (Player p: players)
                p.addScore((120000)); // Give the player 120k pts.
        }

        Sound.bossDeath.play(); // Play boss-death sound.

        Updater.notifyAll("Night Wizard: Defeated!");
        beaten = true;
