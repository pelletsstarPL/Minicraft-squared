...
public class NightWizard extends EnemyMob {
    ...
    public NightWizard(int lvl) {
        super(lvl, sprites, (Updater.isbloody ? 3000 : 2400), false, 16 * 9, -1, 10, 50);
    ...
    @Override
    public void tick() {
        if(!Updater.isbloody && health>maxHealth)health=2400; //overheal bug fix
        super.tick();
            ...
            if (Updater.tickCount % 2100 == 0) {
                ...
                if (level.mobCount + 60 < level.maxMobCount) {
                    ...
                    for (int i = 0; i <= 60; i++) {
                        ...
                        if (rnd < 2)
                            level.add((new Wraith((revenge + 1) > 4 + (Updater.isbloody ? 1 : 0) ? 4 + (Updater.isbloody ? 1 : 0) : (revenge + 1) + (Updater.isbloody ? 1 : 0))), nx, ny);
                        else if (rnd >= 2 && rnd <= 40)
                            level.add((new Slime((revenge + 1) > 4 + (Updater.isbloody ? 1 : 0) ? 4 + (Updater.isbloody ? 1 : 0) : (revenge + 1) + (Updater.isbloody ? 1 : 0))), nx, ny);
                        else if (rnd <= 75)
                            level.add((new Zombie((revenge + 1) > 4 + (Updater.isbloody ? 1 : 0) ? 4 + (Updater.isbloody ? 1 : 0) : (revenge + 1) + (Updater.isbloody ? 1 : 0))), nx, ny);
                        else if (rnd >= 85)
                            level.add((new Skeleton((revenge + 1) > 4 + (Updater.isbloody ? 1 : 0) ? 4 + (Updater.isbloody ? 1 : 0) : (revenge + 1) + (Updater.isbloody ? 1 : 0))), nx, ny);
                    }
                }
            }
            if(Updater.getTime()==Updater.Time.Night) {
        ...
        }else{ this.speed=0;this.burningDuration=0;};
    }
    @Override
    public void doHurt(int damage, Direction attackDir, boolean canCrit) {
        ...
        if (Updater.getTime()!= Updater.Time.Night){
            ...
            boolean crit= random.nextInt(22)>20;
    ...
    public void die() {
        ...
        Game.notifications.add(Updater.isbloody ? "\nAn armor lies on the ground." : "");
        beaten = true;
