...
public class AirWizardPhase3 extends EnemyMob {
	...
	public void tick() {
		...
		if (attackTime > 0) {
			xmov = ymov = 0;
			attackTime *= secondform ? 0.99 : 0.96; // attackTime will decrease by 7% every time and by 3% for second tier phase 1
	...
	public void render(Screen screen) {
		...
		if (AirWizard.invulnerability > 0 && renderTime % 3 == 0) {
			...
			lvlSprites[lvl-1] = (secondform ? MobSprite.compileMobSpriteAnimations(0, 20) : MobSprite.compileMobSpriteAnimations(0, 24));
		}
	...
	public void die() {
		...
		if(!secondform) {
			...
			Game.notifications.add("Rise stronger!");
			level.add(new WraithA(3), x-12, y-12);
			level.add(new WraithA(4), x-12, y+12);
			level.add(new WraithA(4), x+12, y-12);
			level.add(new WraithA(3), x+12, y+12);
			level.add(new AirWizardPhase4(1), x, y);
		} else {
			AirWizard.invulnerability=4;
			Game.notifications.add("Again");
			level.add(new WraithA(3), x-12, y-12);
			level.add(new WraithA(4), x-12, y+12);
			level.add(new WraithA(4), x+12, y-12);
			level.add(new WraithA(3), x+12, y+12);
			level.add(new AirWizardPhase4(2), x, y);
		}
