...
public abstract class Mob extends Entity {
	...
	public void tick() {
		...
		if (level != null && level.getTile(x >> 4, y >> 4) == Tiles.get("lava")) // If we are trying to swim in lava
			// if(!(this instanceof Ghost) && !(this instanceof Wraith) && !(this instanceof WraithA) && !(this instanceof AirWizard) && !(this instanceof AirWizardPhase2) && !(this instanceof AirWizardPhase3) && !(this instanceof AirWizardPhase4) && !(this instanceof AirWizardPhase5) && !(this instanceof NightWizard))
            if(this.canBeAffectedByLava()) {
				this.burningDuration = 560;
				hurt(Tiles.get("lava"), x, y, (!(this instanceof Player) ? 8 : 4)); // Inflict 8 damage to nonplayers, sourced from the lava Tile, with the direction as the opposite of ours.
			}
	...
	public void hurt(Mob mob, int damage) {
		if(mob instanceof Player)Renderer.damageWiggleTime=50*damage;
	...
	public void hurt(Mob mob, int damage, Direction attackDir) { // Hurt the mob, when the source is another mob
		//if(mob instanceof Player)Renderer.damageWiggleTime=5*damage;
	 ...
	 * @param damage The amount of damage to hurt the mob with
	 * @param attackDir The direction this mob was attacked from
	 * @param crittable If this is a crit or not
	 */
	public void hurt(Mob mob, int damage, Direction attackDir,boolean crittable) { // Hurt the mob, when the source is another mob
		//if(mob instanceof Player)Renderer.damageWiggleTime=5*damage;
		if (mob instanceof Player && Game.isMode("creative") && mob != this) doHurt(health, attackDir); // Kill the mob instantly
		else doHurt((int)(damage*(crittable ? 1.75 : 1)), attackDir,crittable); // Call the method that actually performs damage and to check if it is a crit or not
	}
	...
	protected void doHurt(int damage, Direction attackDir,boolean crittable) {
		if (isRemoved() || hurtTime > 0) return; // If the mob has been hurt recently and hasn't cooled down, don't continue
