...
public class EnemyMob extends MobAi {
	...
	public EnemyMob(int lvl, MobSprite[][][] lvlSprites, int health, boolean isFactor, int detectDist, int rwTime, int rwChance) {
		this(lvl, lvlSprites, health, isFactor, detectDist, 180 * Updater.normSpeed, rwTime, rwChance);
	}
	...
	protected void touchedBy(Entity entity) { // If an entity (like the player) touches the enemy mob
		...
		if(Renderer.player.curArmor!=null) {
			if (Renderer.player.curArmor.getName() == "Night armor" && Updater.getTime() == Updater.Time.Night && !Game.isMode("creative"))
				if (!(entity instanceof AirWizard && entity instanceof AirWizardPhase2 && entity instanceof AirWizardPhase3 && entity instanceof AirWizardPhase4 && entity instanceof AirWizardPhase5 && entity instanceof NightWizard) && Math.random() < (Updater.isbloody ? 0.2 : 0.05))
					hurt(this, (int) (dmg + (maxHealth * 0.07)), Direction.NONE);
		}
	}
	...
	public static boolean checkStartPos(Level level, int x, int y) { // Find a place to spawn the mob
		...
		} else if (t != Tiles.get("Stone Door") && t != Tiles.get("Wood Door") && t != Tiles.get("Obsidian Door") && t != Tiles.get("wheat") && t != Tiles.get("farmland") && t != Tiles.get("Skygrass") && t != Tiles.get("Cloud tallgrass")) {
