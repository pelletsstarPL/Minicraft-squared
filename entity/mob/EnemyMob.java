...
public class EnemyMob extends MobAi {
	...
	protected void touchedBy(Entity entity) { // If an entity (like the player) touches the enemy mob
		...
		if(getClosestPlayer().curArmor.getName()=="Night Armor" && Updater.getTime()==Updater.Time.Night && !Game.isMode("creative"))
	...
	public static boolean checkStartPos(Level level, int x, int y) { // Find a place to spawn the mob
		...
		} else if (t != Tiles.get("Stone Door") && t != Tiles.get("Wood Door") && t != Tiles.get("Obsidian Door") && t != Tiles.get("wheat") && t != Tiles.get("farmland") && t != Tiles.get("Skygrass")) {
