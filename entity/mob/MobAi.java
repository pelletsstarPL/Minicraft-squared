...
public abstract class MobAi extends Mob {
	...
	public void tick() {
		...
		if (getLevel() != null) {
			...
			for (Player p: level.getPlayers()) {
				if(p.isWithin(8,this) && p.potionEffects.containsKey(PotionType.AntiTime))this.speed=2*speedS;
				else this.speed=speedS;
	...
	public void doHurt(int damage, Direction attackDir) {
		...
		super.doHurt(damage, attackDir,false);
	}
	public void doHurt(int damage, Direction attackDir,boolean chance) { //if we can crit
		...
		level.add(new TextParticle("" + damage, x, y, (canCrit) ? Color.ORANGE : Color.RED , (canCrit) ? 120 : 60)); // Make a text particle at this position in this level, bright red and displaying the damage inflicted
		if(canCrit)
			level.add(new TextParticle("CRIT!" , x, y, Color.ORANGE,120)); // CRIT!

		super.doHurt(damage, attackDir, chance);
	}
