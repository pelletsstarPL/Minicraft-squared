...
public class Creeper extends EnemyMob {
	...
	public Creeper(int lvl) {
		super(lvl, sprites, lvl>1 && lvl<4 ? 8: (lvl==4 ? 7 : 9), 50);
	}
	...
	public void tick() {
		...
		if (fuseTime > 0) {
			if(fuseTime%5<2)lvlSprites[lvl-1] = (MobSprite.compileMobSpriteAnimations(4, 38));
			else lvlSprites[lvl-1] = (MobSprite.compileMobSpriteAnimations(4, 0+((lvl-1)*2)));
			fuseTime-=getClosestPlayer().potionEffects.containsKey(PotionType.AntiTime) ? 2 : 1; // Fuse getting shorter... and even shorter with antitime
			xmov = ymov = 0;
			...
			if (playerInRange) {
				...
				for (Point tilePosition : tilePositions) { // Destroys tiles in range
					...
					if (!hasSpawner) {
						if (level.depth != 1) {
							level.blow(tilePosition.x, tilePosition.y, 0, Tiles.get("hole"), 0);
						} else {
							level.blow(tilePosition.x, tilePosition.y, 0, Tiles.get("Infinite Fall"), 0);
						}
				...
				lvlSprites[lvl-1] = (MobSprite.compileMobSpriteAnimations(4, 0+((lvl-1)*2)));
				fuseTime = 0;
				fuseLit = false;
