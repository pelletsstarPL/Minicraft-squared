...
import minicraft.entity.Spark4;
...
public class AirWizardPhase4 extends EnemyMob {
	private int artime;
	...
	static {
		...
		// normal sprite
		for (int i = 0; i < 2; i++) {
			MobSprite[][] list = MobSprite.compileMobSpriteAnimations(8, 24 + (i * 2));
			sprites[i] = list;
			invulnerableSprite = MobSprite.compileMobSpriteAnimations(0, 24+ (i*2));
		}
	}
	 ...
	 */
	public AirWizardPhase4(int lvl) {
		this(lvl > 1);
	}
	 ...
	 */
	public AirWizardPhase4(boolean secondform) {
	...
	public void tick() {
		...
		if (attackTime > 0) {
			xmov = ymov = 0;
			attackTime *= secondform ? 0.992 : 0.98; // attackTime will decrease by 7% every time and by 3% for second tier phase 1
			double dir = attackTime * 0.25 * (attackTime % 2 * 2 - 1); // Assigns a local direction variable from the attack time.
			double speed = (secondform ? 1.25 : 0.8); // speed is dependent on the attackType. (higher attackType, faster speeds)
			level.add(new Spark4(this, Math.cos(dir) * speed, Math.sin(dir) * speed)); // Adds a spark entity with the cosine and sine of dir times speed.
	...
	public void render(Screen screen) {
		...
		if (AirWizard.invulnerability > 0 && renderTime % 3 == 0) {
			...
			lvlSprites[lvl-1] = (secondform ? MobSprite.compileMobSpriteAnimations(0, 22) : MobSprite.compileMobSpriteAnimations(0, 24));
		}
	...
	public void die() {
		...
		if(!secondform) {
			...
			AirWizardPhase2.beaten = true;
			AirWizardPhase3.beaten = true;
			AirWizardPhase4.beaten = true;
		} else {
			Sound.fuseChests.play(); // Play boss-death sound.
			AirWizard.invulnerability=6;
			Game.notifications.add("Unleash the wrath!");
			level.add(new WraithA(4), x-12, y-12);
			level.add(new WraithA(4), x-12, y+12);
			level.add(new WraithA(4), x+12, y-12);
			level.add(new WraithA(4), x+12, y+12);
			level.add(new WraithA(4), x-24, y);
			level.add(new WraithA(4), x+24, y);
			level.add(new AirWizardPhase5(2), x, y);
		}
