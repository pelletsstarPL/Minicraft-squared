...
public abstract class Entity implements Tickable {
	...
	protected boolean move2(int xd, int yd) {
		...
		int xt1 = ((x + xd) + xr) >> 4;
		int yt1 = ((y + yd) + yr) >> 4;
		int extraMove=level.getTile(xd==1 ? xt1 : xt0,yd==-1 ? yt1 : yt0).extraSmallHbox;
	//	if(this instanceof Player)System.out.println(x + " " + y + " " + xt1 + " " + yt1 + " " + extraMove + " " + xd + " " + yd);
		//boolean blocked = false; // If the next tile can block you.
		for (int yt = yt0; yt <= yt1+extraMove; yt++) { // Cycles through y's of tile after movement
			for (int xt = xt0; xt <= xt1+extraMove; xt++) { // Cycles through x's of tile after movement
				//extraMove=level.getTile(xt,yt).extraSmallHbox;
				...
				if (!level.getTile(xt, yt).mayPass(level, xt, yt, this)) { // If the entity can't pass this tile...
					if(extraMove==0)return false; //no need to check if hitbox has full size
					//blocked = true; // Then the entity is blocked
					boolean canX=true;
					if(xd>=1){

						canX=(x+xd)<(xt1*16)+extraMove;
						//System.out.println(canX);
					}
					else if(xd==0)canX=false;
					else canX=(x+xd>(xto0*16)+16-extraMove);
					boolean canY=true;
					if(yd>=1)canY=(y+yd<(yto1*16)+extraMove);
					else if(yd==0)canY=false;
					else canY=y+yd>(yto0*16)+16-extraMove;
					if(canX && canY) {
						this.x += canX ? xd : 0;
						this.y += canY ? yd : 0;
					}
