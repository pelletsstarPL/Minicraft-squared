...
public class ItemEntity extends Entity implements ClientTickable {
	...
	public ItemEntity(Item item, int x, int y) {
		...
		lifeTime = 70 * 10 * (item.getDisplayName().contains("Demonicolon") ? 10 : 1) + random.nextInt(70) ; // Sets the lifetime of the item. min = 600 ticks, max = 669 ticks.
	...
	public void tick() {
		...
		xx += gotx - expectedx;
		yy += goty - expectedy;
		if(level.getTile(x >> 4,y >> 4)==Tiles.get("lava") && item.getDisplayName()!="Demonicolon" && !(item instanceof SummonItem) && time>40)remove();
	}
